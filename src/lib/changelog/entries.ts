...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  {
    title: "Official BioHubNet brand — new mark, favicon, brand theme, social card",
    body: "The platform now ships with the official BHN visual identity end-to-end.\n\n**New diamond-cluster mark.** Replaces the old circular medallion in the sidebar, the favicon, and every place LogoMark is used. Built as a tight 64×64 SVG that scales cleanly from 14 px to 400 px — four chevron-striped diamonds in the brand teal / blue / mint palette, reading as both a DNA strand and a network cluster.\n\n**New favicon** at `/icon` (the `src/app/icon.tsx` route) renders the same mark in browsers + bookmarks.\n\n**Daylight theme realigned to brand.** The `light` theme's `brand-N` scale (50 → 900) is now anchored on the official BHN blue `#1A8DB6` instead of the previous generic teal-grey. brand-500 is the brand blue exactly; brand-600 is the AA-safe action colour (5.07:1 against the card surface). Hero gradient sweeps teal → blue → mint to match the wordmark.\n\n**New BioHubNet theme** — the canonical brand expression. Picks up the full tri-colour identity (teal-tinted surfaces, geometric corner radii to echo the diamond mark, all three brand colours in the hero mesh). Pick it from the theme picker; it sits first in the Classic group.\n\n**Open Graph share card.** A new 1200×630 image at `/opengraph-image` renders the mark + tri-colour wordmark + tagline. Any platform link shared on LinkedIn, Slack, Twitter/X, etc. now previews with the real BHN identity.\n\n**Contrast preserved.** Re-ran `scripts/audit-contrast.py` — both `biohubnet` and the updated `light` pass WCAG 2.1 AA (5.07–5.26:1 for the accent on card; 14.65+ for fg). One pre-existing fail on the Scientific theme remains (separate fix, see `docs/ux/audits/2026-05-13-wcag-aa-contrast-sweep.md`).\n\n**Brand colour constants** are now first-class CSS variables: `--bhn-teal #327A80`, `--bhn-blue #1A8DB6`, `--bhn-mint #7FC9A5`. Any surface that wants the full identity (hero gradients, custom cards) can reach for them without re-hardcoding hex values.",
    kind: "feature",
    visibleTo: ALL,
    daysAgo: 0,
  },

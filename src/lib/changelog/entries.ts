...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  ...
  // ── Sidebar · EXPERIENCE trimmed, EQUIP retired, Networking added
  {
    ...
    body: "The sidebar is simpler. **EXPERIENCE** now has a single entry, **Industry Internship**, and **1:1 Mentorship** gains **Networking**. Both are marked *Coming soon* until their pages are ready. The **EQUIP** funding section has been removed from the menu.",
    kind: "improvement",
    visibleTo: ALL,
    daysAgo: 0,
  },
  ...
  // ── Catalogue · renamed, grouped by topic, Specials filter retired
  {
    ...
    body: "Three changes to the catalogue in the sidebar.\n\n**It is called On-Demand Courses now,** in the sidebar and on the page itself.\n\n**Courses are grouped by topic by default.** Sixty-odd cards in one flat grid gave you no way to see what the catalogue actually covers without filtering first. Now each topic gets a large heading, a count and a full-width rule separating it from the topic above — Sector/Technology Overview, Career Insights, Clinical Trials and the rest, in the order the programme teaches them — so you can skim the whole catalogue without touching a control. “All courses” at the top right flattens it back to one list. The headings and that switch stand down when a single topic is filtered, where they would only restate the filter.\n\n**The “Special programs & workshops (instructor-led)” toggle is gone** from the filter panel. Delivery mode still separates the instructor-led series from the asynchronous ones, and admins still mark a course as special from its quick-edit dialog — only the trainee-facing filter button was removed.\n\nAlso fixed while in there: the sidebar’s active item was giving up 26px of label width to a trailing chevron, which truncated “On-Demand Courses” to “On-Demand Cour…” exactly when you were on that page. The chevron was the fourth signal for “you are here” after the fill, the text colour and the left-edge bar; it is gone and every label now fits.",
    kind: "improvement",
    visibleTo: ALL,
    daysAgo: 0,
  },
